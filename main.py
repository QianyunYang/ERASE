...
parser.add_argument('--pretrained_diffusion_path',
                    default="path/to/stable-diffusion-2-1-base",
                    type=str,
                    help='Path to Stable Diffusion model or HuggingFace model ID')
...
parser.add_argument('--bias_mode', default=False, type=bool)
...
def run_diffusion_attack(image, label, diffusion_model, diffusion_steps, guidance=2.5, save_dir="", res=224,
    ...
    for i in range(1):
        if adv_image is None:
            ...
            adv_image, adv_acc, adv_acc1, adv_acc2, adv_acc3, psnr, ssim, processing_time = Diffusion_chain_attack.diffattack(diffusion_model, label,
            ...
            # image = adv_image
            adv_image, adv_acc, adv_acc1, adv_acc2, adv_acc3, psnr, ssim, processing_time = Diffusion_chain_attack.diffattack(
    ...
    return adv_image, adv_acc, adv_acc1, adv_acc2, adv_acc3, psnr, ssim, processing_time
...
if __name__ == "__main__":
    ...
    # 随机选择800个图片
    if len(all_images) > 800:
        all_images = random.sample(all_images, 800)
    ...
    for ind, image_path in enumerate(all_images):
        ...
        classes = [classifier.to('cuda:0') for classifier in classes]
        ldm_stable = ldm_stable.to('cuda:0')
        tmp_image = Image.open(image_path).convert('RGB')
        tmp_image.save(os.path.join(save_dir, str(ind).rjust(4, '0') + "_originImage.png"))
        print("Processing image: {}".format(ind))

        # try:
        adv_image, adv_acc, adv_acc1, adv_acc2, adv_acc3, psnrv, ssimv, processing_time = run_diffusion_attack(tmp_image, label[ind:ind + 1],
                                                                ldm_stable,
    ...
    if processing_times:
        ...
        print(f"\n=== 时间统计 ===")
        print(f"处理图片总数: {len(processing_times)}")
        print(f"总处理时间: {sum(processing_times):.2f} 秒")
        print(f"平均处理时间: {avg_time:.2f} 秒")
        print(f"最快处理时间: {min(processing_times):.2f} 秒")
        print(f"最慢处理时间: {max(processing_times):.2f} 秒")        
