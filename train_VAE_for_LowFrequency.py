...
def image_gradient(img):
    ...
    grad_x = F.conv2d(img, sobel_x.expand(img.size(1),1,3,3), padding=1, groups=img.size(1))
    grad_y = F.conv2d(img, sobel_y.expand(img.size(1),1,3,3), padding=1, groups=img.size(1))
...
class HFLoss(nn.Module):
    ...
    def forward(self, src_magnitude, tgt_magnitude):
        ...
        B, C, H, W = src_magnitude.shape
        mask = torch.ones((H, W), device=src_magnitude.device)
        center = self.size // 2
        mask[center-self.sigma:center+self.sigma, center-self.sigma:center+self.sigma] = 0


        mask = mask.unsqueeze(0).unsqueeze(0)  # [1,1,H,W]
        src_high = src_magnitude * mask
        tgt_high = tgt_magnitude * mask


        loss = torch.sqrt(((src_high - tgt_high) ** 2).sum() / mask.sum())
        return loss
...
if __name__ == '__main__':
        ...
        parser.add_argument("--noise_prototype", type=str, 
                            default="path/to/noise_prototype_VAE.pt")
        parser.add_argument("--dir", type=str,
                            default="path/to/GenImage")
        parser.add_argument("--sigma", type=int, default=8, help="low-frequency region size for LFLoss")
        ...
        # 判别器
        spectral_discriminator = nn.Sequential(
            nn.Linear(3, 1),  # 3为通道数，如果你用环形平均可改为环数
            nn.Sigmoid()
        ).cuda()

        criterionGAN = nn.BCELoss()
        optimizerS = torch.optim.Adam(spectral_discriminator.parameters(), lr=0.0002, betas=(0.5, 0.999))
        radial_profile = RectAverage(args.size, device='cuda')
        ####################################

        ldm_stable = StableDiffusionPipeline.from_pretrained("path/to/stable-diffusion-2-1-base").to('cuda')
        ...
        if not os.path.exists(args.noise_prototype):
            ...
            n_channels = 3
            nb = 20
            filter_path = "path/to/dncnn_color_blind.pth"
            from models.network_dncnn import DnCNN as net
            ...
            print("save noise_prototype to {}".format(args.noise_prototype))
        else:
        ...
        for idx in pbar:
            for idy, batch in enumerate(tqdm(real_dataloader)):
                ...
                src_magnitude = get_magnitude(x_batch)
                rec_magnitude = get_magnitude(rec)
                lf_loss_value = lf_loss(rec_magnitude, src_magnitude)
                print("lf_loss_value: ", lf_loss_value.item())

                loss = lpips_loss + l1_loss + 0.01 * lf_loss_value 
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                if idy % args.freq_log == 0:
                    logger.info("lpips_loss:{}  l1_loss:{} lf_loss:{}".format(lpips_loss, l1_loss, lf_loss_value))
